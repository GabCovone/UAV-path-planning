% This script initializes workspace variables for the UAV Toolbox Reference
% Application.

% Set guidance type to Autotuning
guidanceType = 3;

% Configure the drone as a Multicopter
isDroneMulticopter = 1;

% Use photorealistic environment
isPhotoRealisticSim = 0;

% High fidelity plant model with Autotuning
plantModelFi = 2;

% Show the Lidar Point Cloud
showLidarPointCloud = 0;

% No show the Video Viewer
showVideoViewer = 0;

% Do not show the UAV Animation as it flies
showUAVAnimation = 0;

% Use heading in the guidance model
useHeading = 1;

%Takeoff after 0.005
startFlightTime = 0.005;

% Use QGroundControl
useQGC = 0;

% No Pacing
load_system('uavPIDAutotuning');
set_param('uavPIDAutotuning','EnablePacing', 'off');

% Simulation Stop Time
simTime = 350;

%Show the CPA Scope
close_system('uavPIDAutotuning/On Board Computer/DataProcessing/ProcessSensorData/CPA');

% Done
disp ('Project configured to simulate in a photorealistic environment with high-fidelity plant model PID autotuning');

%% Autotuning Configuration
tunePositionController = 1;
initialGainsMultiplier = 1/4;